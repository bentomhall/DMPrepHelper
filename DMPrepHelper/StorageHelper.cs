...
namespace DMPrepHelper
{
    public class StorageHelper
    {
        ...
        async Task<StorageFile> GetDefaultFile(DataFile fileType)
        {
            ...
            try
            {
                f = await StorageFile.GetFileFromApplicationUriAsync(filename);
        ...
        public async Task ResetConfigToDefault(DataFile type)
        {
            var defaultFile = await GetDefaultFile(type);
            var text = await FileIO.ReadTextAsync(defaultFile);
            var file = await ApplicationData.Current.LocalFolder.CreateFileAsync(dataFiles[type], CreationCollisionOption.ReplaceExisting);
            await FileIO.WriteTextAsync(file, text);
            MarkDirty(type);
            dataText[type] = Task.FromResult(text);
        }
        ...
        private async Task WriteDMGDungeon(Export.ExportWriter e, IEnumerable<DMGDungeon> data)
        {
            var exporter = new Export.DMGDungeonExporter();
            await e.WriteFile(exporter, data);
        }
